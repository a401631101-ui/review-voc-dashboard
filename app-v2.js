...
function tags(title,copy,tone){return `<div class="tag-group ${tone}"><b><i>${funIcon[title]}</i>${title}</b><div>${copy.split(/[；，、]/).filter(Boolean).slice(0,7).map(x=>`<span>${esc(x.trim())}</span>`).join('')}</div></div>`}
function signalTags(title,key,items){const max=Math.max(1,...(items||[]).map(x=>x[1]));return `<div class="signal-card purpose-visual"><h3><i>${icon[key]}</i>${title}</h3><div class="purpose-list">${items?.length?items.map((x,i)=>`<div class="purpose-row"><span class="purpose-symbol">${['🎨','🛡️','🔧','💧','🌿','📦'][i%6]}</span><div><b>${esc(x[0])}</b><span class="micro-track"><i style="width:${Math.max(7,x[1]/max*100)}%"></i></span><small>${fmt(x[1])}条 · ${x[2]}%</small></div></div>`).join(''):'<em>当前筛选未形成明确提及</em>'}</div></div>`}
function themePanel(c,s){
  const list=c.themes.map((t,i)=>{const d=s.themes?.[t[0]]||{};return `<button class="theme-tab" data-i="${i}" aria-current="${i===state.theme}"><span class="theme-glyph">${['🧱','🎨','⚙️','🛠️'][i]}</span><span class="theme-copy"><b>${esc(t[0])}</b><i><u style="width:${Math.min(100,d.rate||0)*2}%"></u></i></span><span>${d.rate||0}%<small>${fmt(d.count)}条</small></span></button>`}).join('');
...
function render(){
  ...
  ${themePanel(c,s)}
  <section class="triple-row">${signalTags('购买目的提及率','purposes',s.purposes)}<div class="signal-card mini-brand"><h3><i>🏁</i>品牌评分对比</h3>${data().shops.map(shop=>{const x=data().segments[`${state.month}|${shop}`];if(!x||x.n<30)return'';return `<div class="brand-line ${shop.includes('雨虹')?'is-rain':''}"><span>${shop.includes('雨虹')?'★ ':''}${esc(shop)}</span><i><u style="width:${x.score/5*100}%"></u></i><b>${x.score.toFixed(1)}</b></div>`}).join('')||'<em>可比样本不足</em>'}</div><div class="signal-card persona-card"><h3><i>🗺️</i>人群 · 场景 · 目的</h3><div class="compact-tags">${[...(s.people||[]),...(s.scenes||[]),...(s.purposes||[])].slice(0,10).map((x,i)=>`<span><i>${['👷','🏡','🎨','🧰','🌿'][i%5]}</i>${esc(x[0])}<small>${x[2]}%</small></span>`).join('')}</div></div></section>
